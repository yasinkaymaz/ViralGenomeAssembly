...
export PATH="/Users/yasinkaymaz/Documents/Tools/plinkseq-0.10":$PATH
echo -e "##phe1,Integer,-9,1/2=eBLs/Controls" > pop.phe
echo -e "##type,Integer,-9,1/2/3=t1/t2/t3" >> pop.phe
cat ~/Dropbox/codes/ViralGenomeAssembly/workspace/data/pop_info.txt|sed 's/ Sequencing//g' |awk '{OFS="\t";if($4 == "eBL")print $0"\t""1"; else print $0"\t""2"}'| cut -f1,3,5 |sed 's/Type//g' >> pop.phe


pseq proj new-project
pseq proj load-vcf --vcf my_ICed_my_edited_sequence.aln.IDreplaced.OnlyIndividuals-MinusCellLines_sub_modified_repeatFiltered.vcf
...
grep -v "#" my_ICed_my_edited_sequence.aln.IDreplaced.OnlyIndividuals-MinusCellLines_sub_modified_repeatFiltered.vcf|wc -l
# Plain
pseq proj v-assoc --phenotype phe1 --perm 1000 --vmeta| gcol VAR REF ALT MINA MINU OBSA OBSU OR P|awk '($9 < 0.01)' |wc -l
# Null genotypes accounted
pseq proj v-assoc --phenotype phe1 --perm 1000 --vmeta --fix-null| gcol VAR REF ALT MINA MINU OBSA OBSU OR P|awk '($9 < 0.01)' |wc -l
# Null genotypes accounted and genotypes are stratified by subtype
pseq proj v-assoc --phenotype phe1 --fix-null --perm 1000 --vmeta --strata type| gcol VAR REF ALT MINA MINU OBSA OBSU OR P|awk '($9 < 0.01)' |wc -l

# Null genotypes accounted and genotypes are stratified by subtype with 1 Million permutation
pseq proj v-assoc --phenotype phe1 --fix-null --perm 1000000 --vmeta --strata type| gcol VAR REF ALT MINA MINU OBSA OBSU OR P|awk '($9 < 0.01)' > 1M.out


pseq proj v-assoc --phenotype type --fix-null --perm 1000 --vmeta --strata phe1| gcol VAR REF ALT MINA MINU OBSA OBSU OR P|awk '($9 < 0.01)' |wc -l


pseq proj glm --phenotype phe1
